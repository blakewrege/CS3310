...
public class UI {
	 ...
	 * @throws IOException
	 */
	public UI(short fileNameSufix, Log log) throws IOException{	
		log.displayThis(String.format(
				"FILE STATUS > TransData FILE opened (A1TransData%s.txt)",
				fileNameSufix));
		input = new Scanner(new File("A2TransData"+fileNameSufix+".txt"));
	}
		...
		if(input.hasNextLine()){
			...
			if (transCode.equals("IN")){
				...
				if (temp.length() > 2){
					...
					try {
						id = Short.parseShort(origId.replaceFirst("^0+(?!$)", ""));
					}catch(Exception e){
						id = 0;
					}
				}
