...
public class CountryDataRec {
	 ...
	 * @throws IOException 
	 */
	public void readCountry(RandomAccessFile file) throws IOException{
		code = "";
		name = "";
		continent = "";
		id = file.readShort();
	 ...
	 * @throws IOException
	 */
	public void writeCountry(RandomAccessFile file,  short id,
	 ...
	 * For calculating and locating byteOffset, and returning the status 
	   of the file at a given address by means of booleans.
	 * @param RRN
	 * @param file
	 * @return spot status.
	 * @throws IOException
	...
	public boolean[] byteOffsetAndStatus(int RRN, RandomAccessFile file){
		...
		try {
			...
			try {
				int test = file.readShort();
				file.seek(byteOffset);
				if (test > 0)
					return new boolean[] { true, false }; /* Existent id. */
				return new boolean[] { false, true }; /* Empty space exists
														 within existent
													     spots. */
			} catch (IOException e) {
				return new boolean[] { true, true };/* New spot; not between
													   others. */
			}
		} catch (IOException e) {
	...
	/********************************************************************
	 * Deletes country from the record by adding 0s.
	 * @param file
	 * @throws IOException 
	 */
	public void deleteCountry(RandomAccessFile file) throws IOException {
